...
Initial <- sapply(fuck, function(x) table(tum.VS.met[V1 == x, -3, with = FALSE])[1])
Common <- sapply(fuck, function(x) unique(table(met.VS.tum[V1 == x, -3, with = FALSE])[2], table(tum.VS.met[V1 == x])[2]))
Recurrence <- sapply(fuck, function(x) table(met.VS.tum[V1 == x, -3, with = FALSE])[1])
